...
#
# THIS FILE IS A TWIN of the one in initialcodess/LogDrop-Android-Analyzer, and the
# two must stay identical from `set -uo pipefail` down. The report is the same SARIF
...
# talk to the same server. The GitHub Action above must break the build on exactly
# the same conditions, and `scripts/verify.sh` in the analyzer repository compares
# all three.
set -uo pipefail
